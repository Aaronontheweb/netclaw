...
public sealed class TurnStateTrackerTests
{
    ...
    [Fact]
    public void Valid_results_do_not_reset_the_invalid_rationale_limit()
    {
        var tracker = new TurnStateTracker();

        tracker.EvaluateInvalidRationaleResults(1, 1);
        Assert.IsType<InvalidRationaleAction.Continue>(
            tracker.EvaluateInvalidRationaleResults(0, 2));
        tracker.EvaluateInvalidRationaleResults(1, 2);
        Assert.IsType<InvalidRationaleAction.Continue>(
            tracker.EvaluateInvalidRationaleResults(0, 1));
        Assert.IsType<InvalidRationaleAction.StopTools>(
            tracker.EvaluateInvalidRationaleResults(1, 1));
    }

    [Fact]
    public void Tool_counter_reset_keeps_the_invalid_rationale_limit()
    {
        var tracker = new TurnStateTracker();

        tracker.EvaluateInvalidRationaleResults(1, 1);
        tracker.EvaluateInvalidRationaleResults(1, 1);
        tracker.ResetToolCounters();
