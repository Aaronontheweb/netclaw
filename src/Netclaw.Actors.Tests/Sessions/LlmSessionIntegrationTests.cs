...
public class LlmSessionIntegrationTests : LlmSessionTestBase
{
    ...
    [Fact]
    public async Task Successful_corrections_do_not_reset_the_invalid_rationale_limit()
    {
        _fakeToolExecutor.RequireRationale = true;
        for (var index = 1; index <= 5; index++)
        {
            ...
            if (index % 2 == 0)
                arguments["_rationale"] = "Load the browser tool for this test.";

            _fakeChatClient.PlannedResponses.Enqueue(
            [
                new FunctionCallContent($"call-{index}", "load_tool", arguments)
            ]);
        }

        var sessionId = new SessionId("signalr/alternating-invalid-rationale-limit");
        var sessionManager = ActorRegistry.Get<SessionManagerActorKey>();
        var subscriber = CreateTestProbe("alternating-invalid-rationale-limit-sub");
        await sessionManager.Ask<SessionJoined>(new JoinSession(subscriber)
        {
            SessionId = sessionId,
            Filter = OutputFilter.Full
        ...
        Assert.Empty(_fakeChatClient.ReceivedToolNames[5]);

        var replayedValidCall = _fakeChatClient.ReceivedMessages[2]
            .SelectMany(message => message.Contents)
            .OfType<FunctionCallContent>()
            .Single(call => call.CallId == "call-2");
        Assert.Equal(
            "Load the browser tool for this test.",
            replayedValidCall.Arguments!["_rationale"]);
    }
