...
A channel-owned inbound HTTP callback endpoint SHALL verify every inbound request before it mutates any session state.
...
buttons. The endpoint SHALL run an ACL evaluation on the resolved sender, and
SHALL be registered only when the owning channel is enabled with interactive
approvals configured. The endpoint SHALL fail closed on invalid configuration
and SHALL NOT create new autonomous sessions — it routes only into existing
sessions identified by the callback payload.
...
- **WHEN** an inbound request arrives with a missing or invalid signature
...
- **GIVEN** a signed callback request resolves to a sender
...
- **GIVEN** a signed, ACL-allowed callback request
