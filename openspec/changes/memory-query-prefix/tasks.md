...
- [ ] 1.1 `EmbeddingPurpose` enum (`Passage`, `RetrievalQuery`) and purpose
...
- [ ] 1.2 `EmbeddingModelManifestEntry` gains `QueryPrefix` (string) and
...
- [ ] 1.3 `OnnxMemoryEmbedder` prepends the manifest `QueryPrefix` for
...
- [ ] 1.4 Holder plumbing: active entry's `QueryPrefix`/
...
- [ ] 2.1 `MemoryRecallConfig.MinCosineSimilarity` becomes nullable; null →
...
- [ ] 2.2 `SQLiteMemoryRecallCoordinator`: effective floor resolved
      ...
      its source
- [ ] 2.3 Doctor: embedding check reports active model's prefix presence and
      effective floor source
- [ ] 2.4 Tests: prefix applied only to `RetrievalQuery` (fixture-model
...
- [ ] 3.1 Record the prefixed fp32 calibration as the calibration of record:
...
- [ ] 3.2 Eval suite run (memory category) on the final behavior
- [ ] 3.3 `netclaw-memory` skill sync: prefix is automatic, floor now
      follows the model manifest by default, override knob semantics; bump
      metadata.version
- [ ] 3.4 Full gates: build, all affected test suites, slopwatch, headers
